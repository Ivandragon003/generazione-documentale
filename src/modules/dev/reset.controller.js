...
async function seedDevFixtures(pool) {
  ...
  await pool.query(
    `INSERT INTO documents (id, name, template_id, template_version, content, field_values, status, created_by)
     VALUES ($1, 'Documento Fixture', $2, 1, $3, $4, 'draft', 'dev-fixture')`,
...
function registerDevRoutes(router) {
  router.post('/test-runs/execute', wrapAsync(async (req, res) => {
    if (process.env.NODE_ENV === 'production') {
      throw { status: 403, message: 'Test run non disponibili in produzione' };
    }

    const suite = req.body?.suite || 'api-regression';
    if (suite !== 'api-regression') {
      throw { status: 400, message: 'Suite non supportata' };
    }
  ...
  router.post('/reset', wrapAsync(async (req, res) => {
    if (process.env.NODE_ENV === 'production') {
      throw { status: 403, message: 'Reset non disponibile in produzione' };
    }

    if (req.headers['x-reset-confirm'] !== 'true') {
      throw { status: 400, message: 'Header x-reset-confirm: true obbligatorio' };
    }
