...
export class TemplatesService {
  ...
  private async hydrateContent<T extends { content_path: string | null }>(
    ...
    if (!content) {
      throw makeError("Contenuto template non disponibile", 500);
    }
  ...
  async create({
    ...
    await this.auditService.log("template", template.id, "create", created_by, {
      name: template.name,
    });
  ...
  async update(
    ...
    const nextContentPath = await this.writeTemplateFile(
      id,
      nextVersion,
      nextContent,
    );
    ...
    await this.auditService.log("template", id, "update", created_by, {
      version: nextVersion,
    });
  ...
  async restore(id: string, targetVersion: number, actor = "system") {
    ...
    if (!oldVersion) {
      throw makeError("Versione template non disponibile", 404);
    }
  ...
  async delete(id: string, actor = "system") {
    ...
    await this.auditService.log("template", id, "delete", actor, {
      name: existing.name,
    });
