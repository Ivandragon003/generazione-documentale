...
export class DocumentsService {
  ...
  async create({
    ...
    await this.auditService.log("document", document.id, "create", created_by, {
      name: document.name,
  ...
  async update(
    ...
    await this.auditService.log("document", id, "update", created_by, {
      version: nextVersion,
    });
    return updated;
  }
  ...
  async enqueuePdfGeneration(id: string, actor = "system") {
    ...
    await this.auditService.log("document", id, "enqueue_pdf", actor, {
      job_id: job.id,
    });
    return job;
  }
  ...
  async restore(id: string, targetVersion: number, actor = "system") {
    ...
    const restored = await this.dataSource.transaction(async (manager) => {
      const row = await this.documentsRepository.restoreDocument(manager, {
        id,
  ...
  async delete(id: string, actor = "system") {
    ...
    await this.auditService.log("document", id, "delete", actor, {
      name: existing.name,
    });
    return { deleted: true };
  }
